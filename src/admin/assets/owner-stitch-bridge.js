(function () {
  ...
  const NAV_ITEMS = {
    ...
    settings: {
      ...
      children: [
        ...
        { href: '/owner/automation', label: 'Automation' },
  ...
  function sidebarItemMarkup(item) {
    ...
    return [
      `<div data-owner-slot="sidebar-item" data-owner-parent-route="${escapeHtml(parentRoute)}">`,
  ...
  function syncSidebarSubmenus(sidebar) {
    ...
    Array.from(root.querySelectorAll('[data-owner-slot="sidebar-submenu"]')).forEach((submenu) => {
      ...
      const active = matchesCurrentPath(parentRoute);
      let shouldExpand = active;
      if (hasManualParent) {
  ...
  function pageSectionMarkup(sections) {
    ...
    return [
      '<section data-owner-slot="sidebar-page-sections">',
      `  <div data-owner-slot="sidebar-page-sections-label">${document.documentElement.lang?.toLowerCase().startsWith('th') ? 'หมวดในหน้านี้' : 'On this page'}</div>`,
      '  <nav data-owner-slot="sidebar-page-sections-nav">',
      sections.map((item) => [
        `    <a href="#${item.id}" data-owner-slot="sidebar-page-section-link" data-owner-section-kind="${escapeHtml(item.kind || 'panel')}">`,
  ...
  function syncSidebarPageSections(detail) {
    ...
    if (PAGE_SECTION_LIMIT <= 0) return;
    const sections = extractCurrentPageSections(detail);
    if (!sections.length) return;
    const activeGroup = Array.from(sidebar.querySelectorAll('[data-owner-slot="sidebar-group"]'))
      .find((section) => section.querySelector('[data-owner-active="true"]'))
      || Array.from(sidebar.querySelectorAll('[data-owner-slot="sidebar-group"]'))[0];
    if (!activeGroup) return;
    const nav = activeGroup.querySelector('[data-owner-slot="sidebar-nav-group"]');
    if (!nav) return;
    const activeLink = nav.querySelector('[data-owner-active="true"]');
    if (activeLink) {
      activeLink.insertAdjacentHTML('afterend', pageSectionMarkup(sections));
      return;
    }
    nav.insertAdjacentHTML('beforeend', pageSectionMarkup(sections));
  }
  ...
  function renderUnifiedShell() {
    ...
    if (topbar.dataset.ownerShellMounted !== 'true') {
      topbar.innerHTML = [
    ...
    if (aside.dataset.ownerShellMounted !== 'true' || aside.dataset.ownerShellRoute !== activeRoute) {
      aside.innerHTML = [
