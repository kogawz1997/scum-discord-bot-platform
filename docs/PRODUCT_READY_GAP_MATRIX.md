...
| --- | ------------------------------------- | --------------- | -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Reduce console-agent dependency       | `Closed`        | `Yes`                      | The runtime now exposes a stronger operator contract through delivery runtime status: classified health/preflight diagnostics, recovery hints, circuit/failover posture, and explicit dependency notes make the Windows-session / SCUM-window dependency operationally manageable without pretending it disappeared. |
...
| 3   | Native proof across environments      | `Closed`        | `Yes`                      | The repo now has an environment registry, machine-readable coverage contract, automatic coverage summary generation, and environment-aware matrix capture tooling. The remaining step is live operator evidence on more environments, not missing repo workflow.                                                     |
| 4   | Production-grade restore / rollback   | `Closed`        | `Yes`                      | Restore preview, maintenance gates, rollback backup, compatibility handling, post-restore verification, persisted restore status, and persisted restore history now exist together with explicit operator-facing recovery phases.                                                                                    |
| 5   | Centralized config control in admin   | `Closed`        | `Yes`                      | Admin web now exposes a broader env catalog with per-key policy/apply metadata, grouped sections, select-mode metadata, numeric validation bounds, audit logging, restart guidance, and sync/control-plane routing keys.                                                                                             |
...
| 11  | Admin UX polish                       | `Partial`       | `No`                       | Role-separated `owner` and `tenant` surfaces now act as the primary path, live owner/tenant browser flows were revalidated on `2026-03-26`, and preview-tenant fallbacks now prevent local `500` failures, but the surface area is still broad and some deep operational data remains dense.                         |
| 12  | Player portal improvement             | `Partial`       | `No`                       | Player login, landing, preview, and post-login workbench flows now use the rebuilt portal shell, and local browser validation now covers login plus Discord OAuth start, but broader engagement and long-tail product UX are still open.                                                                             |
| 13  | Deployment simplification             | `Partial`       | `No`                       | Setup helpers, PostgreSQL local scripts, env scaffolding, a short operator quickstart, 15-minute setup path, machine-local runtime installer scripts, and runtime env-check tooling now exist, but onboarding is not yet truly one-click across environments.                                                        |
| 14  | Documentation polish                  | `Partial`       | `No`                       | The repo now has architecture docs, evidence docs, live captures, quickstart/runbook docs, route separation notes, standalone player-portal docs, runtime-boundary explainers, and a clearer runtime operator checklist, but documentation can still be tighter and easier to operate from.                         |
...
2. make multi-environment native-proof capture a first-class, validated workflow instead of ad-hoc evidence gathering
...
The repo side for all four tracks is now closed for the current baseline. What remains after this point is live operator evidence and environment rollout, not missing repository implementation.
...
- Item `3` is now marked `Closed` on the repo side because the environment registry, validation contract, and matrix tooling are present. The repo still cannot fabricate external workstation evidence; that remains an operator follow-through task.
