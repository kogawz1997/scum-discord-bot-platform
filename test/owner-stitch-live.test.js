...
test('owner stitch live rerenders over server snapshots and marks the shell ready', () => {
  ...
  assert.match(source, /function scheduleRenderWhenNeeded\(\)\s*{[\s\S]*window\.__OWNER_STITCH_SERVER_MODE__ === 'server-rendered'[\s\S]*restoreServerRenderSnapshot\(\)[\s\S]*scheduleRender\(\);[\s\S]*}/);
});

test('owner stitch live route headers expose focus guidance and highlight chips', () => {
  const source = readOwnerStitchLiveSource();

  assert.match(source, /focus:\s*'Start with platform risk, revenue exceptions, and runtime posture before opening a workflow\.'/);
...
test('owner stitch live route headers expose primary and secondary actions', () => {
  const source = readOwnerStitchLiveSource();

  assert.match(source, /function normalizeHeaderActions\(action\)/);
  assert.match(source, /Array\.isArray\(action\?\.secondaryActions\)/);
  assert.match(source, /secondaryActionsForControlRoute\(routeKey\)/);
  assert.match(source, /packages:\s*\[[\s\S]*label: 'Create package'[\s\S]*label: 'Entitlements'/);
  assert.match(source, /runtime:\s*\[[\s\S]*label: 'Create server'[\s\S]*label: 'Provision'/);
  assert.match(source, /settings:\s*\[[\s\S]*label: 'Access policy'[\s\S]*label: 'Managed services'/);
  assert.match(source, /'recovery-restore':\s*\[[\s\S]*label: 'Restore preview'[\s\S]*label: 'History'/);
});
