...
function buildRenderedSchemaBanner(provider, options = {}) {
  const sourcePath = pathToSchemaLiteral(
    path.relative(options.outputDir || PROJECT_ROOT, options.sourcePath || SOURCE_SCHEMA_PATH),
  ) || 'prisma/schema.prisma';
  return [
    '// Generated by scripts/prisma-with-provider.js',
    `// Source schema template: ${sourcePath}`,
    `// Rendered provider: ${provider}`,
    '// Runtime provider truth comes from this rendered schema/client pair, not the literal provider in prisma/schema.prisma alone.',
    '',
  ].join('\n');
}
...
function renderSchemaForProvider(provider, options = {}) {
  ...
  rendered = buildRenderedSchemaBanner(provider, { sourcePath, outputDir }) + rendered;
  ensureDir(outputDir);
