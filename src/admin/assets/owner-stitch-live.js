(function () {
  ...
  function routeVisualPresentation(routeKey) {
    ...
    if (['recovery', 'recovery-create', 'recovery-preview', 'recovery-restore', 'recovery-history', 'backup-detail'].includes(key)) {
      return {
        ...
        focus: 'Review backup readiness, preview checks, and restore history before executing a restore.',
    ...
    if ([
      ...
      return {
        ...
        focus: 'Manage catalog changes, renewal risk, and billing exceptions as separate operator lanes.',
    ...
    if ([
      ...
      return {
        ...
        focus: 'Handle registration, provisioning, and runtime health as separate operational steps.',
    ...
    if (['analytics', 'analytics-risk', 'analytics-packages', 'automation'].includes(key)) {
      return {
        ...
        focus: 'Review signals, automation runs, and risk movement without mixing control actions.',
    ...
    if ([
      ...
      return {
        ...
        focus: 'Inspect audit evidence, access posture, and policy changes in separate control lanes.',
    ...
    return {
      ...
      focus: 'Start with platform risk, revenue exceptions, and runtime posture before opening a workflow.',
  ...
  function normalizeHeaderActions(action) {
    const actions = [];
    const seen = new Set();
    const push = (item) => {
      if (!item || !item.href) return;
      const key = `${String(item.href)}|${String(item.label || '')}`;
    ...
    if (Array.isArray(action)) {
      ...
      push(action);
      (Array.isArray(action?.secondaryActions) ? action.secondaryActions : []).forEach(push);
    }
    return actions;
  }
  ...
  function buildRouteHeaderMarkup(meta, action) {
    ...
    const actionMarkup = normalizeHeaderActions(action)
      .map((item) => `<a href="${escapeHtml(item.href)}" data-owner-ui="action" data-owner-tone="${escapeHtml(item.tone || 'default')}"${item.localFocus ? ' data-owner-local-focus="true"' : ''}>${escapeHtml(item.label || 'Open')}</a>`)
      .join('');
  ...
  function secondaryActionsForControlRoute(routeKey) {
    const mapping = {
      overview: [
        { label: 'Tenant list', href: '/owner/tenants', tone: 'default' },
        { label: 'Runtime overview', href: '/owner/runtime/overview', tone: 'default' },
      ],
      tenants: [
        { label: 'Packages', href: '/owner/packages', tone: 'default' },
        { label: 'Subscriptions', href: '/owner/subscriptions', tone: 'default' },
      ],
      'tenant-create': [
        { label: 'Packages', href: '/owner/packages', tone: 'default' },
      ],
      packages: [
        { label: 'Create package', href: '/owner/packages/create', tone: 'default' },
        { label: 'Entitlements', href: '/owner/packages/entitlements', tone: 'default' },
      ],
      'packages-create': [
        { label: 'Package catalog', href: '/owner/packages', tone: 'default' },
      ],
      'packages-entitlements': [
        { label: 'Package catalog', href: '/owner/packages', tone: 'default' },
        { label: 'Create package', href: '/owner/packages/create', tone: 'default' },
      ...
      subscriptions: [
        { label: 'Registry', href: '/owner/subscriptions/registry', tone: 'default' },
        { label: 'Billing', href: '/owner/billing', tone: 'default' },
      ],
      'subscriptions-registry': [
        { label: 'Renewal queue', href: '/owner/subscriptions', tone: 'default' },
      ],
      billing: [
        { label: 'Recovery', href: '/owner/billing/recovery', tone: 'default' },
        { label: 'Attempts', href: '/owner/billing/attempts', tone: 'default' },
      ],
      'billing-recovery': [
        { label: 'Attempts', href: '/owner/billing/attempts', tone: 'default' },
        { label: 'Invoices', href: '/owner/billing', tone: 'default' },
      ],
      'billing-attempts': [
        { label: 'Recovery', href: '/owner/billing/recovery', tone: 'default' },
        { label: 'Invoices', href: '/owner/billing', tone: 'default' },
      ...
      runtime: [
        { label: 'Create server', href: '/owner/runtime/create-server', tone: 'default' },
        { label: 'Provision', href: '/owner/runtime/provision-runtime', tone: 'default' },
      ],
      'runtime-create-server': [
        { label: 'Provision runtime', href: '/owner/runtime/provision-runtime', tone: 'default' },
      ],
      'runtime-provision-runtime': [
        { label: 'Runtime registry', href: '/owner/runtime/agents-bots', tone: 'default' },
      ],
      'agents-bots': [
        { label: 'Provision runtime', href: '/owner/runtime/provision-runtime', tone: 'default' },
        { label: 'Diagnostics', href: '/owner/runtime/fleet-diagnostics', tone: 'default' },
      ],
      'fleet-diagnostics': [
        { label: 'Runtime registry', href: '/owner/runtime/agents-bots', tone: 'default' },
        { label: 'Incidents', href: '/owner/incidents', tone: 'default' },
      ],
      incidents: [
        { label: 'Support', href: '/owner/support', tone: 'default' },
        { label: 'Jobs', href: '/owner/jobs', tone: 'default' },
      ],
      jobs: [
        { label: 'Automation', href: '/owner/automation', tone: 'default' },
        { label: 'Runtime', href: '/owner/runtime/overview', tone: 'default' },
      ],
      analytics: [
        { label: 'Risk queue', href: '/owner/analytics/risk', tone: 'default' },
        { label: 'Package usage', href: '/owner/analytics/packages', tone: 'default' },
      ],
      'analytics-risk': [
        { label: 'Analytics', href: '/owner/analytics/overview', tone: 'default' },
        { label: 'Support', href: '/owner/support', tone: 'default' },
      ],
      'analytics-packages': [
        { label: 'Analytics', href: '/owner/analytics/overview', tone: 'default' },
      ...
      automation: [
        { label: 'Jobs', href: '/owner/jobs', tone: 'default' },
        { label: 'Recovery', href: '/owner/recovery/overview', tone: 'default' },
      ],
      support: [
        { label: 'Diagnostics', href: '/owner/diagnostics', tone: 'default' },
      ],
      audit: [
        { label: 'Security', href: '/owner/security/overview', tone: 'default' },
        { label: 'Access', href: '/owner/access', tone: 'default' },
      ],
      security: [
        { label: 'Audit', href: '/owner/audit', tone: 'default' },
        { label: 'Access', href: '/owner/access', tone: 'default' },
      ],
      access: [
        { label: 'Security', href: '/owner/security/overview', tone: 'default' },
      ...
      settings: [
        { label: 'Access policy', href: '/owner/settings/access-policy', tone: 'default' },
        { label: 'Managed services', href: '/owner/settings/services', tone: 'default' },
      ],
      'settings-admin-users': [
        { label: 'Access policy', href: '/owner/settings/access-policy', tone: 'default' },
        { label: 'Managed services', href: '/owner/settings/services', tone: 'default' },
      ...
      'settings-services': [
        { label: 'Platform controls', href: '/owner/control', tone: 'default' },
        { label: 'Runtime policy', href: '/owner/settings/runtime-policy', tone: 'default' },
      ],
      'settings-access-policy': [
        { label: 'Portal policy', href: '/owner/settings/portal-policy', tone: 'default' },
        { label: 'Admin users', href: '/owner/settings/admin-users', tone: 'default' },
      ],
      'settings-portal-policy': [
        { label: 'Billing policy', href: '/owner/settings/billing-policy', tone: 'default' },
        { label: 'Access policy', href: '/owner/settings/access-policy', tone: 'default' },
      ],
      'settings-billing-policy': [
        { label: 'Runtime policy', href: '/owner/settings/runtime-policy', tone: 'default' },
      ...
      recovery: [
        { label: 'Create backup', href: '/owner/recovery/create', tone: 'default' },
        { label: 'Restore preview', href: '/owner/recovery/preview', tone: 'default' },
      ],
      'recovery-create': [
        { label: 'Restore preview', href: '/owner/recovery/preview', tone: 'default' },
      ],
      'recovery-preview': [
        { label: 'Apply restore', href: '/owner/recovery/restore', tone: 'default' },
      ],
      'recovery-restore': [
        { label: 'Restore preview', href: '/owner/recovery/preview', tone: 'default' },
        { label: 'History', href: '/owner/recovery/history', tone: 'default' },
      ],
      'recovery-history': [
        { label: 'Create backup', href: '/owner/recovery/create', tone: 'default' },
        { label: 'Recovery overview', href: '/owner/recovery/overview', tone: 'default' },
      ],
    };
    return mapping[routeKey] || [
      { label: 'Owner overview', href: '/owner', tone: 'default' },
    ];
  }
  ...
  function actionForControlRoute(pathname, routeKey) {
    ...
    const primary = mapping[routeKey] || { label: 'Back to overview', href: '/owner', tone: 'default' };
    return {
      ...primary,
      secondaryActions: secondaryActionsForControlRoute(routeKey),
    };
  }
